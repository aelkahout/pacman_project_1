...
class CornersProblem(search.SearchProblem):
    ...
    def expand(self, state):
        ...
        self._expanded += 1 # DO NOT CHANGE
        # children = (nextNode, nextAction, nextCost)
        return children
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # Calculate all distances from goals(0 for starting at goal state)
    goalDistance = [0]
    for index, item in enumerate(state[1]):
        if item == 0:  # Not visited corner
            # Manhattan distance used
            heurDistance = util.manhattanDistance(state[0], corners[index])
            goalDistance.append(heurDistance)
    # Returning highest possible distance
    return max(goalDistance)
...
def foodHeuristic(state, problem):
    ...
    goalDistance = [0]
    #print(foodCoord)
    foodCoord.insert(0, position)
    foodCoord.sort()
    for i in range(len(foodCoord) -1):
        goalDistance.append(mazeDistance(state[0], foodCoord[-1], problem.startingGameState))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #both BFS and Astar search should give the closest path
